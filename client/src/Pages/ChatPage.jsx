...
const ChatPage = () => {
  ...
  if(isLoadingMyMatches) return null;
    ...
    <div className='flex flex-col h-screen bg-gray-100 bg-opacity-50' >
      ...
      <div className='grow flex flex-col p-4 md:p-6 lg:p-8 overflow-hidden max-w-4xl mx-auto w-full' >
       ...
       <div className='grow overflow-y-auto mb-4 bg-white rounded-lg shadow p-4' >
             {messages.length === 0 ? (
              <p className='text-center text-gray-500 py-8'>Start your conversations with {match.name}</p>
             ) : (
              messages.map((msg) => (
                <div key={msg._id}
                ...
                >
                  ...
                  className={`inline-block p-3 rounded-lg max-w-xs lg:max-w-md ${
                    msg.sender === authUser._id ? "bg-pink-500 text-white" : "bg-gray-200 text-gray-800"
                  }`}
                  >
                    {msg.content}
                  </span>
...
const MatchNotFound = () => (
  <div className='h-screen flex flex-col items-center justify-center bg-gray-100 bg-opacity-50 bg-dot-pattern' >
      <div className='bg-white p-8 rounded-lg shadow-md text-center' >
        ...
        <p className='text-gray-600' >Oops! it seems match doesn&apos;t exist or has been removed.</p>
        <Link to='/'
